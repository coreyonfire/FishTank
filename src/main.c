...
static void anim_started_handler(Animation *animation, void *context) {
  ...
  if (from.x - to.x < 0) {
    // fish is going right
    bitmap_layer_set_bitmap(layer, gbitmap_create_with_resource(RESOURCE_ID_ORANGE_FISH_RIGHT));
  }
  else {
    // fish is going left
    bitmap_layer_set_bitmap(layer, gbitmap_create_with_resource(RESOURCE_ID_ORANGE_FISH_LEFT));
  }
}
...
static void main_window_load(Window *window) {
  ...
  // Create fish bitmaps
  s_fish_bitmap = gbitmap_create_with_resource(RESOURCE_ID_ORANGE_FISH_RIGHT);
  s_fish_bitmap2 = gbitmap_create_with_resource(RESOURCE_ID_ORANGE_FISH_RIGHT);
  s_fish_bitmap3 = gbitmap_create_with_resource(RESOURCE_ID_ORANGE_FISH_RIGHT);
  ...
  s_time_layer = text_layer_create(GRect(0, 30, 144, 70));
